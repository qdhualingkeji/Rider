...
public class DaiSongDaNode implements OnGetRoutePlanResultListener {
    ...
    private DaiSongDaAdapter daiSongDaAdapter;
    private boolean haveCD=false;

    public DaiSongDaNode(DaiSongDaAdapter daiSongDaAdapter){
        // 初始化搜索模块，注册事件监听
        mSearch = RoutePlanSearch.newInstance();
        mSearch.setOnGetRoutePlanResultListener(this);
        this.daiSongDaAdapter =daiSongDaAdapter;
    }
    ...
    public void initKm(){
        ...
        List<DaiSongDaEntity.DataBean> mData = daiSongDaAdapter.getmData();
        //Log.e("ToQc=,ToSc=111111==",toQcdjl+","+toScdjl);
        for (int i=0;i<mData.size();i++) {
            DaiSongDaEntity.DataBean dataBean = mData.get(i);
        ...
        //}
        DaiSongDaAdapter.jiSuanPosition++;
        if(DaiSongDaAdapter.jiSuanPosition==daiSongDaAdapter.getDsdNodeList().size()) {//当已计算出的数量等于适配器里集合的元素数量时，适配器再发出第二次通知，刷新出路程来
            daiSongDaAdapter.notifyDataSetChanged();
            DaiSongDaAdapter.jiSuanPosition = 0;//发出通知之后，把数量标志清零
